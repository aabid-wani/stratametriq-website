  ...
  return (
    <div id="showcase" className="py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      {filteredProducts.length === 0 ? (
        <div className="text-center py-16 rounded-3xl bg-obsidian-800/40 border border-glass">
          <Sparkles className="w-12 h-12 text-gray-500 mx-auto mb-4 animate-pulse" />
          <h3 className="text-lg font-bold text-white">No Suites in this Category</h3>
          <p className="text-sm text-gray-400 mt-1">Select another filter or view all software products.</p>
        </div>
      ) : (
          ...
          {filteredProducts.map((product) => (
            <div
              ...
              className={`rounded-3xl p-7 flex flex-col justify-between transition-all duration-300 relative overflow-hidden group ${
                product.isFeatured
                  ? "bg-gradient-to-b from-obsidian-800/90 to-obsidian-900/95 border-2 border-electric-500/30 hover:border-electric-400 shadow-glass"
                  : "bg-obsidian-800/60 hover:bg-obsidian-800/90 border border-glass hover:border-glass-hover shadow-glass"
              }`}
            >
              ...
              <div>
                ...
                <div className="flex items-start justify-between mb-6">
                  <div className="p-3.5 rounded-2xl bg-obsidian-950 border border-glass shadow-md group-hover:scale-110 transition-transform duration-300">
                    {getIcon(product.iconName)}
                  </div>
                  <div className="flex flex-col items-end space-y-1">
                    <div className="flex items-center space-x-2">
                      <span className="px-2 py-0.5 rounded-full text-[11px] font-bold bg-white/5 text-gray-300 border border-glass">
                        {product.badge}
                      </span>
                    ...
                    {product.downloadsCount && (
                      <span className="text-[11px] text-gray-400 flex items-center font-mono">
                        <Download className="w-3 h-3 mr-1 text-neon-cyan" />
                ...
                <Link href={product.href}>
                  <h3 className="text-xl font-extrabold text-white group-hover:text-electric-400 transition-colors leading-snug">
                    {product.name}
                  </h3>
                </Link>

                {/* Tagline & Description */}
                <p className="text-sm font-medium text-gray-300 mt-2.5 leading-relaxed">
                  {product.tagline}
                </p>
                ...
                {product.installCommand && (
                  <div className="mt-5 p-2.5 rounded-xl bg-obsidian-950/90 border border-glass flex items-center justify-between text-xs font-mono text-gray-300 group/cmd hover:border-glass-hover transition-colors">
                    <div className="flex items-center space-x-2 overflow-hidden mr-2">
                      <Terminal className="w-3.5 h-3.5 text-electric-400 flex-shrink-0" />
              ...
              <div className="mt-8 pt-5 border-t border-glass/60 flex flex-col space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  <Link
                    href={product.href}
                    className="px-4 py-2.5 rounded-xl border border-glass hover:border-glass-hover bg-white/5 hover:bg-white/10 text-white text-xs font-bold text-center flex items-center justify-center space-x-1.5 transition-all"
                  >
                  ...
                  {product.hasLiveDemo ? (
                    ...
                    <Link
                      href={product.href}
                      className="px-4 py-2.5 rounded-xl bg-obsidian-950 border border-glass text-gray-400 hover:text-white text-xs font-bold text-center flex items-center justify-center transition-all"
                    >
